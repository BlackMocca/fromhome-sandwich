-- ============================================================
-- DASHBOARD VIEWS — v5 (fix net_profit formula)
-- ------------------------------------------------------------
-- แก้สูตร net_profit ในทั้ง 2 views ให้ตรงตาม business logic:
--
--   net_profit = grand_total (net_sales)
--                − total_cost (ต้นทุนสินค้าที่ขาย)
--                − claim_loss (ต้นทุนของเสีย เคลม, status='active')
--
-- หมายเหตุ:
--   • ingredient_cost ยังคงอยู่ใน view เพื่อแสดงผลแยก (เช่น bar chart)
--     แต่ "ไม่" ถูกหักออกจาก net_profit อีกต่อไป
--   • เหตุผล: net_profit ใช้วัดผลกำไรจากการขายจริง ๆ ส่วน ingredient_cost
--     เป็นต้นทุนการลงทุน (ไม่ใช่ต้นทุนสินค้าที่ขาย)
-- ============================================================

-- ============================================================
-- 1. view_daily_summary
-- ============================================================
...
SELECT
  ...
  (COALESCE(SUM(h.grand_total), 0) -
   ...
   COALESCE(c.claim_cost, 0))::NUMERIC(12,2)          AS net_profit
FROM hdr h
...
COMMENT ON VIEW view_daily_summary IS
  'Per-day, per-channel sales/cost summary. net_profit = grand_total - total_cost - claim_loss (active claims only). ingredient_cost exposed separately, not subtracted from net_profit.';
COMMENT ON VIEW view_monthly_sales_profit IS
  'Per-month sales/profit trend. net_profit = grand_total - total_cost - claim_loss (active claims only). ingredient_cost exposed separately, not subtracted from net_profit.';
