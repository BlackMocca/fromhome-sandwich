...
import { PlusCircle, AlertCircle, CheckCircle2, Eye, X } from 'lucide-react';
...
import { getActiveCategories, createProduct, saveProductAddonMappings, getActiveProductAddons } from '@/lib/db';
...
export default function CreateProductPage() {
  ...
  const handleSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      // Save product-addon mappings if any selected
      if (selectedAddonIds.length > 0) {
        await saveProductAddonMappings(result.id, selectedAddonIds);
      }
      ...
      setBanner({ kind: 'success', text: 'สร้างสินค้าเรียบร้อยแล้ว' });
      ...
      toast({
        ...
        description: err instanceof Error ? err.message : 'ไม่สามารถสร้างสินค้าได้',
      });
      setBanner({
        kind: 'error',
        text: err instanceof Error ? err.message : 'ไม่สามารถสร้างสินค้าได้',
      });
    } finally {
  ...
  const formik = useFormik<FormValues>({
    initialValues: { name: '', category_id: categories.length > 0 ? categories[0].id : 0, cover_url: '', base_price: 0, cost: 0 },
  ...
  const previewProduct: any = {
    id: 0,
    name: formik.values.name || 'ชื่อสินค้าตัวอย่าง',
    category_id: formik.values.category_id || 0,
    cover_url: formik.values.cover_url || null,
    base_price: formik.values.base_price || 0,
    cost: formik.values.cost || 0,
    is_active: true,
  };

  const previewCategory = categories.find(c => c.id === formik.values.category_id) || null;
      ...
      <Card className="border border-primary/20 bg-white shadow-sm">
        <CardHeader className="space-y-1.5 pb-2">
          ...
          <CardDescription className="text-center text-muted-foreground">
            สร้างข้อมูลสินค้า (Product) สำหรับจัดการเมนูและราคา
          </CardDescription>
        </CardHeader>
        ...
        <CardContent className="space-y-5 pt-2">
          ...
          <form onSubmit={formik.handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center justify-between pt-4 border-t border-border/50 mt-6">
              <Button type="button" variant="ghost" onClick={() => setIsPreviewOpen(true)} disabled={!formik.isValid}>
