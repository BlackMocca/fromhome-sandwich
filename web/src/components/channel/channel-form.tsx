...
import { PlusCircle, Edit2, AlertCircle, CheckCircle2 } from 'lucide-react';

import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';
import ImageUpload from '@/components/ui/image-upload';
import { create, update, getOne } from '@/lib/db';
...
export function ChannelForm({ mode, channelId }: { mode: ChannelFormMode; channelId?: string }) {
  ...
  /* ─── Fetch existing channel (edit only) ─── */
  useEffect(() => {
    if (!isCreate && channelId) {
      getOne<Channel>('channels', channelId)
        .then(setChannel)
        .catch((err) => {
          setBanner({
            kind: 'error',
            text: err instanceof Error ? err.message : 'ไม่สามารถโหลดข้อมูลช่องทางได้',
          });
        });
    }
  }, [isCreate, channelId]);
  ...
  const handleSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (isCreate) {
        await create('channels', payload);
      } else {
        if (!channel?.id) throw new Error('ไม่พบข้อมูลช่องทาง');
        await update('channels', channel.id, payload);
      }

      queryClient.invalidateQueries({ queryKey: ['channels'] });

      const successText = isCreate ? 'สร้างช่องทางเรียบร้อยแล้ว' : 'แก้ไขช่องทางเรียบร้อยแล้ว';
      toast({ title: 'สำเร็จ!', description: successText });
      ...
      const errorText = err instanceof Error ? err.message : (isCreate ? 'ไม่สามารถสร้างช่องทางได้' : 'ไม่สามารถแก้ไขช่องทางได้');
      toast({ title: 'เกิดข้อผิดพลาด', description: errorText });
      ...
      <Card className="border border-primary/20 bg-white shadow-sm">
        <CardHeader className="space-y-1.5 pb-2">
          <CardTitle className="text-xl font-bold text-primary text-center">
            {isCreate ? 'เพิ่มช่องทางใหม่' : 'แก้ไขช่องทาง'}
          </CardTitle>
          <CardDescription className="text-center text-muted-foreground">
            {isCreate ? 'สร้างช่องทางการขายสำหรับจัดกลุ่มสินค้า' : 'แก้ไขข้อมูลช่องทางการขาย'}
          </CardDescription>
          ...
          <div className="space-y-4">
            ...
            <div className="flex text-white items-center justify-end gap-2 pt-2">
              ...
              >
                ...
                {isSubmitting ? (isCreate ? 'กำลังสร้าง...' : 'กำลังบันทึก...') : (isCreate ? 'เพิ่มช่องทาง' : 'บันทึก')}
              </Button>
            </div>
          </div>
        </CardContent>
